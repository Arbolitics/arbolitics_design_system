...
module ArboliticsDesignSystem
  module Generators
    class InstallGenerator < Rails::Generators::Base
      desc "Adds arbolitics_design_system stylesheet and CLAUDE.md guidance to the host app"

      def inject_stylesheet
        layout = "app/views/layouts/application.html.erb"
        return unless File.exist?(layout)

        if File.read(layout).include?("arbolitics_design_system")
          say "  stylesheet already present in #{layout}", :yellow
        else
          inject_into_file layout, before: "</head>" do
            %(    <%= stylesheet_link_tag "arbolitics_design_system" %>\n)
          end
      ...
      def append_claude_md
        claude_md = "CLAUDE.md"
        marker    = "## Arbolitics Design System"
      ...
      def claude_md_content
        <<~MD
          ...
          UI components are provided by the `arbolitics_design_system` gem.
          ...
          - The stylesheet is `arbolitics_design_system.css` — already
